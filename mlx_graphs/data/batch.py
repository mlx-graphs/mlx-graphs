...
class Batch(GraphData):
    ...
    def __init__(self, node_features, edge_index, num_nodes) -> None:
        self.node_features = node_features
        self.edge_index = edge_index
        self.cumsum_num_nodes = mx.cumsum(num_nodes)
    ...
    def from_graph_list(cls, graphs: List[GraphData]) -> Self :
        ...
        global_edge_index = [graph.edge_index + num_nodes[i] for i,graph in enumerate(graphs)]
        global_edge_index = mx.concatenate(global_edge_index, axis=1)

        return cls(global_node_features, global_edge_index, num_nodes = num_nodes)
    ...
    def __getitem__(self, idx):
        ...
        lower_last_bound = self.cumsum_num_nodes[idx].item()
        upper_bound = self.cumsum_num_nodes[idx + 1].item()
        node_features = self.node_features[lower_last_bound:upper_bound]

        mask = (self.edge_index >= lower_last_bound) &\
                (self.edge_index <=upper_bound)

...
CombineRootFunctions = Literal["add", "cat", "self_loop"]
...
class SimpleConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if combine_root_func not in get_args(CombineRootFunctions):
            raise ValueError(
    ...
    def __call__(
        ...
        if self.combine_root_func is not None:
            if self.combine_root_func == "add":
                output = output + node_features
            elif self.combine_root_func == "cat":
                output = mx.cat([node_features, output], dim=-1)

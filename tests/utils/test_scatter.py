...
def test_scatter():
    ...
    assert mx.array_equal(
        y_hat1, mx.array([0.5, 0.5, 1, 1])
    ), "Simple scatter softmax failed"
    assert mx.array_equal(y_hat2, mx.array([2, 1, 1])), "Simple scatter add failed"
    assert mx.array_equal(y_hat3, mx.array([1, 1, 1])), "Simple scatter max failed"
    assert mx.array_equal(y_hat4, mx.array([1, 1, 1])), "Simple scatter mean failed"
...
def test_scatter_2D():
    ...
    assert mx.array_equal(
        mx.round(y_hat1, 3), mx.round(y1, 3)
    ), "scatter 2D softmax failed"
    assert mx.array_equal(y_hat2, y2), "scatter 2D add failed"
    assert mx.array_equal(y_hat3, y3), "scatter 2D max failed"
    assert mx.array_equal(y_hat4, y4), "scatter 2D mean failed"
